...
from app.services.strategy_allocation import (
    allocation_ratio,
    choose_ballast_symbol,
    choose_defense_symbol,
    choose_growth_satellite_symbol,
    choose_one_x_symbol,
    clamp,
    defensive_ratio,
    leverage_rotation_model,
    normalize_ratios,
    one_x_equity_ratio,
    one_x_selection_reason,
    ratio_map,
    recommended_ballast_ratio,
    risk_band,
    symbol_if_allowed,
    target_effective_leverage_by_disparity,
    tqqq_cap_from_disparity,
)
...
def build_buy_plan(
    ...
    if target <= 0 or regime == "risk_off":
        return [
            SplitStep(
                ...
                ratio_of_target=0,
                amount=0,
                note="방어 구간에서는 TQQQ 신규 매수를 보류하고 SGOV/CASH를 우선합니다.",
            )
        ]
    ...
    triggers = [
        "전략 선택 후 1차 진입",
        "2차: QQQ 20일선 근처 눌림 또는 200일선 대비 +8% 이하 이격 완화",
        "3차: QQQ 50일선 방어 확인 또는 200일선 대비 +5% 이하 이격 완화",
    ]
    notes = [
        "추세 참여를 시작하는 초기 TQQQ 집행입니다. 이 단계도 이격도별 레버리지 상한 안에서만 실행합니다.",
        "분할매수는 수익률 엔진이 아니라 추격매수를 막는 실행 규율입니다. 조건이 오지 않으면 무기한 현금 대기보다 QQQM/SPYM 1x 완충으로 참여를 유지합니다.",
        "마지막 TQQQ 집행은 깊은 눌림 또는 이격 완화가 확인될 때만 씁니다. 조건이 불명확하면 1x 완충 또는 SGOV/CASH 역할을 유지합니다.",
    ]
    return [
        SplitStep(
            step=f"{index + 1}차 매수",
...
def build_rule_based_report(
    ...
    return CoachReport(
        ...
        monitoring_rules=[
            ...
            "월 1회 TQQQ, 1x 완충 ETF, SGOV/CASH의 역할이 현재 시장 국면과 맞는지 확인합니다.",
        ],
    )
